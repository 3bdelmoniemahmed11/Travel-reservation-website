...
const Main = ({ PriceValue, filteration }) => {
  ...
  const filter = () => {
    if (parseInt(PriceValue.current) === 0) {
      setTravelInfo(Data);
    } else {
      const filterdTravels = TravelsInfo.filter(
        (el) => el.fees === PriceValue.current
      );
      setTravelInfo(filterdTravels);
    }
  };

  const displayedData = TravelsInfo.map(
  ...
  return (
    <section className="main" id="main">
      <div className="container">
        <div onClick={filter} data-aos="fade-right" className="mainTitle ">
